...
const Products = () => {
  ...
  axios.get(`https://dummyjson.com/products/category/${slug}`)
    .then(res => setCatePro(res.data.products))
            ...
            <div className="container">
                 <div className="pt-[71px]">
                    <div className="flex items-end pb-[80px]">
                                     <div>
                                        <h4 className='text-[16px] font-poppin font-semibold leading-[20px] text-[#DB4444] pl-[36px] relative after:w-[20px] after:h-[40px] after:absolute after:content-[""] after:top-[-10px] after:left-0 after:bg-red-500 after:rounded-sm'>Category</h4>
                                        <h2 className='text-[36px] pt-[34px] leading-[48px] font-inter font-semibold text-[#DB4444]'>{slug}</h2>
                                    </div>
                 ...
                 <div className="grid grid-cols-4  gap-[30px]">
                            {
                                catepro.slice(0,visibleCount).map((pro , index) => (
                                 ...
                                 <div className=' bg-[#F5F5F5] py-[35px] px-[40px] rounded-sm mb-4 relative overflow-hidden group'>
                                     <Link to={`/product/details/${pro.id}`}><img className='mx-auto' src={pro.thumbnail} alt="" /></Link>
